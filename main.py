...
MODEL_PATH = os.path.join(os.path.dirname(__file__), "hand_tracker.bin")
CAM_WIDTH, CAM_HEIGHT = 640, 480
SCREEN_WIDTH, SCREEN_HEIGHT = pyautogui.size()

# SMOOTHING PARAMETERS (ADAPTIVE)
SMOOTH_FACTOR_SLOW = 7  # High smoothing for precision
SMOOTH_FACTOR_FAST = 2  # Low smoothing for speed
SPEED_THRESHOLD = 15     # Pixel distance to switch to fast mode
...
# UI COLORS (MODERN)
CLR_ACCENT = (255, 0, 255)  # Magenta
CLR_SUCCESS = (0, 255, 127) # Spring Green
CLR_BG = (30, 30, 30)       # Dark Gray
...
# --- INITIALIZE HAND TRACKER (LIVE STREAM MODE) ---
...
def draw_hud(img, action, fps):
    # Modern HUD with rounded feel (using thin rectangles)
    h, w, _ = img.shape
...
while cap.isOpened():
    ...
    if latest_result and latest_result.hand_landmarks:
        for landmarks in latest_result.hand_landmarks:
            ...
            # 1. BUTTER-SMOOTH ADAPTIVE CURSOR
            # Interpolate Index coordinates
            ...
            smoothing = np.interp(move_dist, (0, SPEED_THRESHOLD), (SMOOTH_FACTOR_SLOW, SMOOTH_FACTOR_FAST))
            ...
            # Right Click
                ...
                if time.time() - last_click_time > DEBOUNCE_TIME:
                    ...
                    last_click_time = time.time()
                    last_action = "R-CLICK"
                cv2.circle(img, (int(middle.x * CAM_WIDTH), int(middle.y * CAM_HEIGHT)), 25, (255, 50, 50), 2)
            ...
            # Scroll
                ...
                cv2.circle(img, (int(middle.x * CAM_WIDTH), int(middle.y * CAM_HEIGHT)), 20, (255, 255, 0), 2)
